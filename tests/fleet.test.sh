...
test_fleet_prints_a_block_per_workspace_and_a_line_per_repo() {
  ...
  case "$out" in *"widget"*"orch LIVE"*) ;; *) echo "widget was not LIVE: $out"; _fleet_teardown; return 1;; esac
  case "$out" in *"gadget"*"orch -"*) ;; *) echo "gadget was not '-': $out"; _fleet_teardown; return 1;; esac
...
test_fleet_json_carries_the_same_numbers_and_workspace_limits_the_scope() {
  ...
  local u; u="$(printf '%s' "$doc" | jq -c '.workspaces[0].units[] | select(.name=="widget")')"
...
test_fleet_survives_herdr_being_down() {
  _fleet_setup
  find "$WT" -exec touch -d '2 hours ago' {} + 2>/dev/null || true
  local doc; doc="$(STUB_HERDR_FAIL=1 cmd_fleet --json)"
